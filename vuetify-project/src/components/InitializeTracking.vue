                    ...
                    <v-card title="Step Two" flat>
                        <v-row>
                            <v-col cols="12" class="text-center">
                                ...
                                <p>To authorise your Tado account, click the link below and follow the instructions and
                        ...
                        <v-row>
                            <v-col cols="12" class="text-center">
                                ...
                                <div v-if="!ActualDataIsVisible">
                                    <p>This should show some data to prove correctness and an interval to specify for
                                        tracking.</p>
                                    <p>Click Finish to complete the initialisation process and start tracking your Tado
                                        temperatures.</p>
                                </div>
                                <div v-else>
                                    <p>We successfully retrieved the Tado data. See here some actual values:</p>
                                    <li>The Zone Name is {{ ActualTadoData?.zoneName }}</li>
                                    <li>The temperature is {{ ActualTadoData?.insideTemperatureCelsius }} degrees
                                        Celsius.</li>
                                    <li>The Humidity is {{ ActualTadoData?.humidityPercentage }} %</li>
                                    <li>The (Tado) Home ID is {{ ActualTadoData?.homeId }}</li>
                                    <li>The Token is stored in our database and the ID is {{ ActualTadoData?.tokenId }}
                                    </li>
                                    <p>Set the tracking interval and click Finish to complete the initialisation process
                                        and start tracking your Tado temperatures.</p>
                                    <v-slider v-model="slider" :max="max" :min="min" :step="1" class="align-center"
